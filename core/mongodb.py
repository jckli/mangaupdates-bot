...
class Mongo:
    ...
    async def add_add_role_server(self, server_id, discord_role_id):
        self.srv.update_one({"serverid": server_id}, {"$addToSet": {"add_roles": discord_role_id}})
    ...
    async def get_server_add_roles(self, server_id):
        result = self.srv.find_one({'serverid': server_id}, {'add_roles': 1})
        return result.get('add_roles', [])
    ...
    async def remove_add_role_server(self, server_id, discord_role_id):
        self.srv.update_one({"serverid": server_id}, {"$pull": {"add_roles": discord_role_id}})
